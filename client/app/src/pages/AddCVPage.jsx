...
function AddCV() {
  ...
  useEffect(() => {
    if (cvId && cvId !== '0') {
      console.log('Fetching data for cvId:', cvId);
      axios.get(`http://127.0.0.1:8000/api/cv/${cvId}/`)
        .then(response => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (cvId) {
        await axios.put(`http://127.0.0.1:8000/api/cv/${cvId}/`, payload);
        alert('CV updated successfully!');
        navigate('/view-records')
      } else {
        await axios.post('http://127.0.0.1:8000/api/cv/', payload);
